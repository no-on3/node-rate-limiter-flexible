...
class RateLimiterSQLite extends RateLimiterStoreAbstract {
  ...
  constructor(opts, cb = null) {
    ...
    if (!/^[A-Za-z0-9_]*$/.test(this.tableName)) {
      throw new Error("Table name must contain only letters and numbers");
    }
          ...
          this.client.run(
            ...
            (err) => {
              if (err) {
                ...
                resolve({ points, expire: newExpire });
              }
            }
          );
        } else {
          // Increment points if exists and not expired, otherwise insert
          this.client.run(
            `
            ...
            (err) => {
              ...
              this.client.get(
                ...
                [rlKey],
                (finalStateErr, result) => {
                  if (finalStateErr) reject(finalStateErr);
                  else resolve(result);
                }
              );
            }
        ...
        if (forceExpire) {
          ...
          this.client.get(
            ...
            (err, row) => {
              ...
              const params = row ? [points, rlKey] : [rlKey, points, newExpire];
